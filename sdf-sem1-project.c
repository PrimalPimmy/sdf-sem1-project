...

{
    ...
    do
    {
        ...
        //Take a string "hidden" to print the value of "word" and print them on basis of whether they are true or not//
        int N = strlen(word);
        int hidden[N];
        for (int i = 0; i < N; ++i)
        {
        ...
        while (!gameover)
        {
            //Loop to print * in place of the letter if not guessed yet using fallacy and tautology//
            printf("The word is : ");
            for (int j = 0; j < N; ++j)
            {
                if (hidden[j])
            ...
            //Input of letter and loop to determine the status of "hidden " by equating it to the guess//
            char guess;
            printf("Letter? ");
            scanf(" %c", &guess);
...
void Roulette()
{
    ...
    { //Main Menu
        ...
        scanf("%lf", &bet);
        printf("Would you like to bet on a specific number(N) or on odd/even(O):\n");
        scanf(" %c", &gametype);
